    ...
    if (rsp.Response.Student) {
      [].concat(rsp.Response.Student || []).forEach(student => {
        if (student.DailyLifeScore.TextScore) {
          ...
          score.set(`OtherRecommend_文字描述`,data.OtherRecommend ? data.OtherRecommend.Description : '');
          score.set(`Origin_OtherRecommend_文字描述`,data.OtherRecommend ? data.OtherRecommend.Description : '');
