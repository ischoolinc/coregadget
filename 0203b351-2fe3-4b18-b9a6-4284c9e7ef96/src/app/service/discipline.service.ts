...
export class DisciplineService {
  ...
  async getDisciplineContract() {
    if (!this._contract) {
      if (this.basicsrv.systemPosition === 'teacher') {
        this._contract = await this.gadget.getContract('ischool.discipline_competition.teacher');
      } else {
        this._contract = await this.gadget.getContract('ischool.discipline_competition.student');
      }
    }
  }
  ...
  async getMyScoreSheetByDate(date: string, class_id: string) {
    await this.getDisciplineContract();
