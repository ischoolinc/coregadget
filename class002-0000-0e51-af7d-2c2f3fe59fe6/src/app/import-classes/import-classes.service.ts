...
export class ImportClassesService {
  ...
  mappingKeys = [
    ...
    { name: 'TeacherFullName', mapping: ['teacher_full_name', '班導師'] },
  ];
  ...
  public makeRules(mode: ImportMode, identifyField: ClassFieldName[], importField: ClassFieldName[], sourceClasses: ClassRec[]) {
    ...
    switch (mode) {
      case 'ADD':
        ...
        if (importField.includes('TeacherFullName')) {
          rowRules.push(new MatchAndReset({ fieldNames: ['TeacherFullName'],
            data: this.coreSrv.teacherList.map(v => { return { ...v, TeacherFullName: (v.Nickname) ? `${v.TeacherName}(${v.Nickname})`: v.TeacherName }})
            , skipEmpty: true, message: '資料庫無符合內容'
            , callback: (rowValue, matchValue: TeacherRec) => { rowValue.TeacherId = matchValue.TeacherId }
          }));
        }
