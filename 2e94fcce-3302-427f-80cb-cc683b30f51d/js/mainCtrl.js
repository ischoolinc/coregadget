angular.module('app', ['ui.sortable', 'mgcrea.ngStrap.helpers.dimensions', 'mgcrea.ngStrap.helpers.debounce', 'monospaced.elastic', 'moment-picker'])
    ...
    .controller('MainCtrl', ['$scope', '$timeout',
        function ($scope, $timeout) {
            ...
            var scope = {
                ...
                omeVisitTypeOption: ["本人對談", "電話聯絡", "聯絡簿", "家長來訪", "其他"],
                    ...
                    result: function (response, error, http) {
                        $scope.$apply(function () {
                            if (!response)
                                ...
                                stuRec.HomeVisitRecord.forEach(function (rec) {
                                    rec.HomeVisitDateString = dateFormat(new Date( parseInt(rec.HomeVisitDate)));
                                    rec.HomeVisitDateMoment = moment(parseInt(rec.HomeVisitDate));
                                    rec.IsReferral = (rec.IsReferral == "true");
                                    rec.IsPrivate = (rec.IsPrivate == "true");
                                    rec.IsMine = (rec.IsMine == "true");
            ...
            $scope.ShowHomeVisitEditor = function (rec) {
                if (!rec) {
                    rec = {
                        ...
                        HomeVisitDateMoment: moment(),
                        ContactName: $scope.CurrentStudent.StudentName,
            ...
            $scope.DeleteHomeVisitRecord = function (rec) {
                ...
                if (strconfirm == true) {
                    gadget.getContract('1campus.counsel.teacher').send({
                        service: "DeleteHomeVisitingRecord",
                        body: { HomeVisitingRecord: { UID: rec.UID } },
                        result: function (response, error, http) {
                            if (!response)
                                alert("DeleteHomeVisitingRecord" + ' Error' + JSON.stringify(error));
                            else {
                                $scope.$apply(function () {
                                    //重新讀取
            ...
            $scope.SaveHomeVisit = function (record) {
                ...
                if (!rec.HomeVisitDateMoment) {
                    err += (err ? '\n' : '') + "請填寫訪談日期!";
                }
                else {
                    rec.HomeVisitDate = rec.HomeVisitDateMoment.format("YYYY/MM/DD");
                }

                if (!rec.CounselTypeKind)
                    err += (err ? '\n' : '') + "訪談方式不得空白!";
                ...
                // 移除暫存的屬性
                delete rec.HomeVisitDateMoment;
                delete rec.AttendeesOption ;
                delete rec.ContactOption ;
                delete rec.CounselTypeKindOption ;

                gadget.getContract('1campus.counsel.teacher').send({
                    service: rec.UID ? "PutHomeVisitingRecord" : "PushHomeVisitingRecord",
                    body: { HomeVisitingRecord: rec },
                    result: function (response, error, http) {
                        if (!response)
                            alert((rec.UID ? "PutHomeVisitingRecord" : "PushHomeVisitingRecord") + ' Error' + JSON.stringify(error));
                        else {
                            $scope.$apply(function () {
                                //重新讀取
                                $scope.SetCurrentView('HomeVisit');
                                ...
                                result: function (response, error, http) {
                                    $scope.$apply(function () {
                                        if (!response) {
                                            ...
                                            //從 DB 中取出來的排序有問題，所以重新排一次
                                            $scope.ConselStudent.sort((stu1, stu2) => {
                                                return (parseInt(stu1.SeatNo || "0") - parseInt(stu2.SeatNo || 0));
                                            });
